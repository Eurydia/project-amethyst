...
import { tauriPostDriverReportMedical } from "$backend/database/post";
...
const TITLE_COLUMN_DEFINITION: TableHeaderDefinition<DriverReportEntry> = {
	...
	render: (item) => (
		<BaseTypographyLink to={"/drivers/report/medical/info/" + item.id}>
			{item.title}
		</BaseTypographyLink>
	),
};
...
export const DriverReportMedicalTable: FC<DriverReportMedicalTableProps> = (
	props
) => {
					...
					slotProps={{
						submitButton: {
							...
							onClick: (formData: DriverReportFormData) =>
								tauriPostDriverReportMedical(formData)
									.then(
										() => {
