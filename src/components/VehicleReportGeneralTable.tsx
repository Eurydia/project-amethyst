...
const TITLE_HEADER_DEFINITION: TableHeaderDefinition<VehicleReportGeneralEntry> =
	{
		...
		render: ({ id, title }) => (
			<BaseTypographyLink to={"/vehicles/report/general/info/" + id}>
				{title}
			</BaseTypographyLink>
		),
	};
...
export const VehicleReportGeneralTable: FC<VehicleReportGeneralTableProps> = (
	props
) => {
				...
				<VehicleReportGeneralForm
					...
					slotProps={{
						submitButton: {
							...
							onClick: (formData: VehicleReportGeneralFormData) =>
