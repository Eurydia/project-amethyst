...
const VEHICLE_HEADER: TableHeaderDefinition<VehicleReportInspectionEntry> = {
	...
	render: ({ vehicleId, vehicleLicensePlate }) => (
		<BaseTypographyLink to={"/vehicles/info/" + vehicleId}>
			{vehicleLicensePlate}
		</BaseTypographyLink>
	),
};
const TITLE_HEADER: TableHeaderDefinition<VehicleReportInspectionEntry> = {
	...
	compare: (a, b) => a.inspectionRoundNumber - b.inspectionRoundNumber,
	render: ({ inspectionRoundNumber, id }) => (
		<BaseTypographyLink to={"/vehicles/report/inspection/info/" + id}>
...
> = (props) => {
	...
	let headers = [DATETIME_HEADER, VEHICLE_HEADER, TITLE_HEADER, TOPIC_HEADER];
				...
				<VehicleReportInspectionForm
					...
					slotProps={{
						submitButton: {
							...
							onClick: (formData: VehicleReportInspectionFormData) =>
								tauriPostVehicleReportInspection(formData)
									.then(
										() => {
											toast.success("เพิ่มผลการตรวจสภาพสำเร็จ");
											revalidate();
										},
										() => toast.error("เพิ่มผลการตรวจสภาพล้มเหลว")
									)
