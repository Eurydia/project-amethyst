...
> = (props) => {
	const { report, inspectionRoundNumber, vehicle, slotProps } = props;
	...
	const infoItems = [
		...
		{
			label: "ลงบันทึกเมื่อ",
			value: dayjs(report.datetime).locale("th").format("HH:mm น. DD MMMM YYYY"),
		},
		{
			label: "เลขทะเบียน",
			value: (
	...
	infoItems.push({
		...
		value:
			report.content.trim().length === 0 ? (
				<Typography fontStyle="italic">ไม่มีรายละเอียดเพิ่มเติม</Typography>
			) : (
			...
			<VehicleReportInspectionForm
				...
				slotProps={{
					submitButton: {
						...
						onClick: (formData) =>
							tauriPutVehicleReportInspection(report.id, formData)
								.then(
									() => {
										toast.success("บันทึกการเปลี่ยนแปลงสำเร็จ");
										revalidate();
									},
									() => toast.error("บันทึกการเปลี่ยนแปลงล้มเหลว")
								)
